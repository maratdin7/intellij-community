...
import org.jetbrains.kotlin.idea.analyser.expression.Expression.Fun
import org.jetbrains.kotlin.idea.analyser.expression.Expression.TerminatedExpression.Constant
import org.jetbrains.kotlin.idea.analyser.expression.Expression.TerminatedExpression.SomeCoroutines
...
class AnalyserVisitor(private val map: MutableMap<KtFunction, Expression>) : KtVisitor<Expression, Nothing?>() {
    override fun visitKtElement(element: KtElement, data: Nothing?): Expression {
        ...
        var numOfCoroutines: Expression = Constant.ZERO
        while (child != null) {
    ...
    override fun visitNamedFunction(function: KtNamedFunction, data: Nothing?): Expression {
        map.getOrPut(function) { visitKtElement(function, data) }
        return Constant.ZERO
    }

    override fun visitLambdaExpression(expression: KtLambdaExpression, data: Nothing?): Expression = Constant.ZERO

    override fun visitTryExpression(expression: KtTryExpression, data: Nothing?): Expression {
        return SomeCoroutines
    }

    override fun visitWhenExpression(expression: KtWhenExpression, data: Nothing?): Expression =
        expression.run {
            val entriesExpr = checkBranches(entries) ?: SomeCoroutines
            val subjExpr = subjectExpression?.accept(this@AnalyserVisitor, data) ?: Constant.ZERO
            subjExpr + entriesExpr
        }

    override fun visitWhenEntry(ktWhenEntry: KtWhenEntry, data: Nothing?): Expression =
        ktWhenEntry.run {
            val condExpr = conditions.all { it.accept(this@AnalyserVisitor, data) == Constant.ZERO }
            if (condExpr) expression?.accept(this@AnalyserVisitor, data) ?: Constant.ZERO
            else SomeCoroutines
        }

    override fun visitIfExpression(expression: KtIfExpression, data: Nothing?): Expression =
        expression.run {
            val branchExpr = checkBranches(listOf(then, `else`)) ?: return SomeCoroutines
            val condExpr = condition?.accept(this@AnalyserVisitor, data) ?: Constant.ZERO
            condExpr + branchExpr
        }
    ...
    private fun checkBranches(branches: List<KtElement?>): Expression? {
        ...
        if (set.isEmpty()) return Constant.ZERO
    ...
    override fun visitCallExpression(expression: KtCallExpression, data: Nothing?): Expression {
        analyze(expression) {
            ...
            val ktFunction = reference?.resolve() as? KtFunction ?: return SomeCoroutines

            val name = ktFunction.fqName?.asString() ?: ""
            val withLambda = analyseWithLambda(name)

            val functionCall =
                expression.resolveCall().successfulFunctionCallOrNull() ?: return SomeCoroutines

            val argsKtExpr = functionCall.argumentMapping.keys

            val argsExpr = argsKtExpr.fold(Constant.ZERO as Expression) { acc, ktExpression ->
                val expr =
            ...
            val coroutinesInFunction = when {
                isCreateCoroutine(name) -> Constant.ONE
                isAllowedStdlibFun(name) -> Constant.ZERO
                else -> Fun(ktFunction)
            }
    ...
    companion object {
        ...
        fun isCreateCoroutine(name: String) = name.isNameAllowed(coroutinePackage, funCreatedCoroutines)

        fun isAllowedStdlibFun(name: String) = name.isNameAllowed(kotlinStdlibPackage, stdlibFunWhereLambdaGuaranteedWillExecuted)

        fun analyseWithLambda(name: String): Boolean =
            name.run {
                isAllowedStdlibFun(this)
                        || isCreateCoroutine(this)
